...
window.addEventListener('scroll',()=>{ if(window.scrollY>400){ btt.classList.add('show'); btt.style.opacity=1; } else { btt.classList.remove('show'); btt.style.opacity=0.25; } });
btt.addEventListener('click',()=>window.scrollTo({top:0,behavior:'smooth'}));

// Theme toggle (two-state) with flip animation
(function(){
  ...
  apply(saved);
  btn.addEventListener("click",()=>{ const next=localStorage.getItem(key)==="light"?"dark":"light"; localStorage.setItem(key,next); apply(next); btn.style.animation='flipIcon .6s ease'; setTimeout(()=>btn.style.animation='',600); });
})();
...
const dict={
  en:{
    ...
    aboutText:"I started my retail journey at 18. I’ve worked with various products — from spare parts and photo equipment to modern vape products. There were pauses, but each return brought a higher level of responsibility. Retail for me is about systems: finding growth points, optimizing processes, and strengthening teams. Today, at 25, I lead the vape category and assortment across 150+ locations. I build SOPs, run promotions, own KPIs, and grow teams while staying people‑first and results‑driven.",
...
(function(){try{const saved=localStorage.getItem(LANG_KEY);if(saved==='en'){document.documentElement.setAttribute('lang','en');langBtn.textContent='EN / 🇺🇦';for(const [k,v] of Object.entries(dict.en)){document.querySelectorAll('[data-i18n="'+k+'"]').forEach(el=> el.textContent=v);}}}catch(e){}})();
